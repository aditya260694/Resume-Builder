...
@auth.requires_membership('Student')
def edit_student():
    #studentid = int(request.args(0)) # or int(request.vars.studentid)
...
@auth.requires_membership('Student')
def view_msg():
    r=db((auth.user_id == db.details.myid) & (db.details.id == db.msg.name)).select(db.msg.sender, db.msg.sub, db.msg.msg)
    dict_companies={}
    for i in r:
